...
const getProductById = async id => {
  try {
    ...
    const result = titles
      .map((h2, index) => {
        const title = h2.innerHTML.replace(/\s+$/, '').toLowerCase().replace(/\s+/g, '_');
        const value = {};
        if (specifications.getElementsByClassName('product-table')[index] !== undefined) {
          const ProductTable = specifications.getElementsByClassName('product-table')[index];
          if (ProductTable.getElementsByTagName('tbody')[0] !== undefined) {
            ...
            Tbody.getElementsByTagName('tr').forEach(tr => {
              ...
              if (tr.getElementsByTagName('td').length === 2) {
                let value2 = '';
                const value1 = tr.getElementsByTagName('td')[0].innerHTML.replace(/\s+$/, '').toLowerCase().replace(/\s+/g, '_');
                value2 = tr.getElementsByTagName('td')[1].innerHTML.replace(/\s+$/, '');
                if (tr.getElementsByTagName('td')[1].getElementsByTagName('a').length === 1) {
                  const a = tr.getElementsByTagName('td')[1].getElementsByTagName('a')[0];
        ...
        if (specifications.getElementsByClassName('included-parts').length > 0) {
          const IncludedParts = specifications.getElementsByClassName('included-parts')[index];
          return {
